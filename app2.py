...
st.markdown("### Integrated: Article Sync, Stock Warnings, and Manual Clustering")
...
# --- 2. MANUELL KLUSTERING-FUNKTION (Istället för scikit-learn) ---
def run_manual_kmeans(df, features, k=3, iterations=10):
    """Egen implementation av K-Means för att slippa externa bibliotek"""
    # 1. Skala data (Min-Max Scaling)
    ...
    # 2. Välj slumpmässiga startpunkter (centroids)
    np.random.seed(42) # Gör det repeterbart
    idx = np.random.choice(len(scaled_data), k, replace=False)
    centroids = scaled_data[idx]

    for _ in range(iterations):
        # 3. Beräkna avstånd till varje centroid (Euclidean distance)
        # diff: (antal_länder, k, antal_features)
        diff = scaled_data[:, np.newaxis] - centroids
        distances = np.sqrt((diff**2).sum(axis=2))
        
        # 4. Tilldela varje land till närmaste centroid
        labels = np.argmin(distances, axis=1)
        
        # 5. Uppdatera centroids (medelvärdet av punkterna i klustret)
        new_centroids = np.array([scaled_data[labels == i].mean(axis=0) 
                                 if len(scaled_data[labels == i]) > 0 else centroids[i] 
                                 for i in range(k)])
...
if z_marketing and stock_file:
    ...
    if country_col:
        ...
        if len(df_c_input) >= 3:
            # Kör vår egen manuella klustring istället för sklearn
    ...
    with st.sidebar:
        ...
        selected_clusters = st.multiselect("Visa prestanda för kluster:", options=unique_clusters, default=unique_clusters, format_func=lambda x: f"Kluster {x}" if x != -1 else "Okända länder")
    ...
    df_s_pivot = df_s_raw.groupby('Article').agg({**{col: 'sum' for col in stock_cols}, 'Season': 'first', 'Partner_Article_Variant': 'first'}).reset_index()
    df_s_pivot['Total_Stock'] = df_s_pivot[stock_cols].sum(axis=1)
    df_s_valid = df_s_pivot[df_s_pivot['Total_Stock'] > 0].copy()

    # E. Merge & Tiering
    df = pd.merge(df_m_agg, df_s_valid[['Article', 'Total_Stock', 'Season']], on='Article', how='left').fillna({'Total_Stock': 0})
    ...
    with tab2:
        ...
        # Hämtar brand från inventory om möjligt
        df_brand = pd.merge(df, df_s_raw[['Article', 'brand']].drop_duplicates(), on='Article', how='left').fillna({'brand': 'Unknown'})
    ...
    with tab4:
        st.header("🌍 Landsklustring (Manuell K-Means)")
        if not df_country_summary.empty:
            st.info("Klustring utförd med inbyggd matematik utan externa bibliotek.")
